...
import { Delete, CornerDownLeft, X, GripHorizontal } from 'lucide-react'
import { KEYBOARD_LANGUAGES, KEYBOARD_LAYOUTS } from '../lib/keyboardLayouts'

export default function VirtualKeyboard({ lang, setLang, onChar, onBackspace, onSpace, onEnter, onClose }) {
  ...
  return (
    <div
      className="fixed z-50 w-[min(92vw,32rem)] rounded-2xl border border-gray-200 bg-white shadow-2xl"
      style={{ right: '1.5rem', bottom: '7rem', transform: `translate(${pos.x}px, ${pos.y}px)` }}
    >
      <div
        onPointerDown={handlePointerDown}
        onPointerMove={handlePointerMove}
        onPointerUp={handlePointerUp}
        className="flex cursor-grab items-center justify-between gap-2 rounded-t-2xl border-b border-gray-100 bg-gray-50 px-3 py-2 select-none active:cursor-grabbing"
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="flex h-6 w-6 items-center justify-center rounded-full text-gray-400 hover:bg-gray-200 hover:text-gray-700"
        >
          ...
          {KEYBOARD_LANGUAGES.map((l) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                lang === l.code ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              {l.label}
            </button>
